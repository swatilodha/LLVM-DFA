...
public:
  ...
  virtual bool runOnFunction(Function &F) {
    ...
    dataFlow *df = new dataFlow(definitions.size(), UNION, FORWARD,
    ...
    for (BasicBlock &BB : F) {
      ...
      for (BasicBlock::iterator II = BB.begin(); II != BB.end(); ++II) {
        ...
        if (BinaryOperator *BO1 = dyn_cast<BinaryOperator>(I)) {
          ...
          // Calculate kill[BB] : Set of definitions in the domain that are
          // killed by the current Basic Block
