...
void dataFlow::getConnectedBlocks(BasicBlock *BB,
                                  struct basicBlockProps *basicBlock) {
...
void dataFlow::executeDataFlowPass(
    Function &F, std::map<BasicBlock *, basicBlockDeps *> bbSets) {
  ...
  std::vector<BasicBlock *> blockTraverse =
      (passDir == FORWARD) ? rpoTraversal : poTraversal;
  while (!hasConverged) {
    for (BasicBlock *BB : blockTraverse) {
      prevOutputs[BB] = dataFlowHash[BB]->bbOutput;
      std::vector<BitVector> meetVectors;
      if (passDir == FORWARD) {
        for (std::vector<BasicBlock *>::iterator f =
                 dataFlowHash[BB]->predBlocks.begin();
             f != dataFlowHash[BB]->predBlocks.end(); ++f) {
          meetVectors.push_back(dataFlowHash[*f]->bbOutput);
        }
      } else if (passDir == BACKWARD) {
        for (std::vector<BasicBlock *>::iterator b =
                 dataFlowHash[BB]->succBlocks.begin();
             b != dataFlowHash[BB]->succBlocks.end(); ++b) {
    ...
    hasConverged = true;
    for (std::map<BasicBlock *, BitVector>::iterator it = prevOutputs.begin();
         it != prevOutputs.end(); ++it) {
...
void dataFlow::printMapping() {
  ...
  while (it != dataFlowHash.end()) {
    ...
    for (int n = 0; n < (int)temp->killSet.size(); n++)
      outs() << temp->killSet[n] << ' ';
    outs() << '\n';
    for (std::vector<BasicBlock *>::iterator i = temp->predBlocks.begin();
         i != temp->predBlocks.end(); ++i) {
      outs() << "pred " << *i << "\n";
    }
    for (std::vector<BasicBlock *>::iterator j = temp->succBlocks.begin();
         j != temp->succBlocks.end(); ++j) {
      outs() << "succ " << *j << "\n";
    }
    it++;
  }
