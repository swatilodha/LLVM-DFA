...
public:
  ...
  virtual bool runOnFunction(Function &F) {
    ...
    for (Function::arg_iterator A = F.arg_begin(); A != F.arg_end();
         ++A) // arguments iterator
    {
      Value *arg = (Value *)A;
      values.push_back(arg);
      variables.push_back((string)arg->getName());
    }
    for (BasicBlock &BB : F) {
      for (Instruction &II : BB) {
        Instruction *I = &II;
        for (User::op_iterator i = I->op_begin(), j = I->op_end(); i != j;
             ++i) {
          Value *var = *i;
...
private:
        ...
        for (int opc = 0; opc < (int)I->getNumOperands(); ++opc) {
          ...
          if (PHINode *phi = dyn_cast<PHINode>(I)) {
            for (int z = 0; z < (int)phi->getNumIncomingValues(); ++z) {
              ...
              if (std::find(domain.begin(), domain.end(), phi_val->getName()) !=
                  domain.end()) {
                phiValues.set(domainMap[(string)phi_val->getName()]);
              }
            }
          }
          if (std::find(domain.begin(), domain.end(), op->getName()) !=
              domain.end()) {
            bbSet->genSet.set(domainMap[(string)op->getName()]);
          }
        }
        if (std::find(domain.begin(), domain.end(), I->getName()) !=
            domain.end()) {
          bbSet->killSet.set(domainMap[(string)I->getName()]);
          if (bbSet->genSet[domainMap[(string)I->getName()]]) {
            bbSet->genSet.reset(domainMap[(string)I->getName()]);
          }
        }
      }
      bbSets[BB] = bbSet;
  ...
  void printDFAResults(std::map<BasicBlock *, basicBlockProps *> dFAHash) {
    ...
    while (it != dFAHash.end()) {
      ...
      // temp->genSet &= phiValues;   // Remove Values corressponding to Phi
      // Nodes
      outs() << "use[BB] : ";
      ...
      // temp->killSet &= phiValues;  // Remove Values corressponding to Phi
      // Nodes
      outs() << "def[BB] : ";
      ...
      // temp->bbInput &= phiValues;  // Remove Values corressponding to Phi
      // Nodes
      outs() << "IN[BB] : ";
      ...
      // temp->bbOutput &= phiValues;  // Remove Values corressponding to Phi
      // Nodes
      outs() << "OUT[BB] : ";
